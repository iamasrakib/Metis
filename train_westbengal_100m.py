#!/usr/bin/env python
"""
Train a 100M parameter Metis model on the West Bengal dataset.

Architecture:
  d_model=768, n_heads=16, n_kv_heads=4 (GQA), n_layers=16
  SwiGLU FFN (hidden≈2048), max_seq_len=256

Hardware target: RTX 2050 (4 GB VRAM)
  - AMP (bf16/fp16) for fused attention kernels
  - Gradient checkpointing (requires --no-cuda-graphs)
  - micro_batch_size=2, gradient_accumulation=8 → effective batch 16
"""
import os
import sys
import time
...
config = ModelConfig(
    ...
    max_iters=30,               # Short demo run (CPU); use 500+ on GPU
    learning_rate=3e-4,
    min_lr=1e-5,
    weight_decay=0.1,
    warmup_steps=5,
    ...
    checkpoint_dir="checkpoints_westbengal_100m",
    tokenizer="char",
    use_mmap=True,
    num_workers=0,
...
def main():
    print("=" * 70)
    print("  Metis 100M — West Bengal Training Run")
    print("=" * 70)
